...
# publishes std_msgs to the 'chatter' topic as Int32
...
def talker():
    ...
    while not rospy.is_shutdown():
        ...
        weather_info_str = weather_msgs(zip = zip_code, location = format(data['city']) +', ' + format(data['country']), temperature = data['temp'], sky = format(data['sky']), lastUpdate = format(datetime.datetime.fromtimestamp(int(data['dt'])).strftime('%I:%M %p')))
